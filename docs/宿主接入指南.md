...
>      IA（左键/滚轮）+ IMC（优先级 2）→ Owner Actor `EnableInput` → 绑定转发到
>      `InteractionComponent->PressPointerKey/ScrollWheel`，世界面片像普通 UI 一样
>      可点击、可滚轮；
>    - 输入模式统一走 `AShellProjectPlayerController::SetShellUIFocus(true)`：
...
| Tab | **宿主**（双管道：面板焦点时经 OnHostFlowKey 委托；无焦点时走 IMC_Shell 游戏输入） | 循环呈现态（Shrink→Occluded→Hand→InputWindow）；面板焦点下不触发命令补全 |
| ESC | **宿主**（双管道同上） | InputWindow 态退出回 Shrink；其余态忽略 |
...
| 5.2.1 世界终端屏幕 | `Source/UE_Shell_Project/Screen/ShellWorldScreen.h/.cpp`（`UShellWorldScreen`）、`Screen/ShellWorldScreenActor.h/.cpp`（`AShellWorldScreenActor`） |
...
"世界终端屏"是三层组成，迁移对象与方式不同——**插件和组件是"搬"，接线是"抄"，配置是"重做"**。
...
| 世界屏组件（`Source/.../Screen/`） | `UShellWorldScreen`、`AShellWorldScreenActor`、`UShellWidgetInteractionComponent` | **直接复制源码**（孤立可用），Build.cs 补模块依赖：`EnhancedInput、UMG、Shell_UE、InputCore`（+ Private `Slate/SlateCore`） |
...
"登录页世界屏 + 可打字可点击"：插件整体搬 + Screen 三类复制 + 对方开始关卡摆一个 `AShellWorldScreenActor` + BeginPlay 调 `ShowLoginScreen`（内部已完成 SetInputActive/聚焦/输入模式）——半天内可跑通。Gameplay 四态呈现机（Tab 循环/手持姿态动画）是可选增强，按需抄。
