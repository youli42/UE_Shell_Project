...
- 下一 tick 校验 `UShellSessionSubsystem::IsLoggedIn()`，未登录 `ClientTravel`
  弹回菜单；
- 已登录创建 `UShellSessionHUD`（登录态标签）加入视口。输入模式/光标策略统一由
  `AShellProjectPlayerController` 拥有（`SetShellUIFocus`；OnPossess 应用默认呈现态
  即 `GameOnly` + 隐藏光标），GameMode 不再直接设置。
...
3. `SetShellUIFocus` 模式：`GameAndUI + SetHideCursorDuringCapture(false)` ↔ `GameOnly`——输入模式/光标策略唯一所有权；
