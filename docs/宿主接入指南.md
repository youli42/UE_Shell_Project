...
| --- | --- | --- | --- | --- |
| `Gameplay` | `GameOnly` | 隐藏 | 无 | **HeldInHand**（手持面板，输入接管关闭）；`SetShellUIFocus(false)` |
| `UiBrowse` | `GameAndUI` | 显示 | 无（不抢键盘，WASD 保留） | 悬浮快捷菜单开但无活动打字面；`SetShellUIFocus(true)` 且无打字面 |
...
- **InputWindow → `UiTyping`**：`ApplyShellPresentation` 在面前态
  `RequestState(UiTyping)`——焦点进终端文本框可打字；
- **HeldInHand → `Gameplay`**：同函数在手持态 `RequestState(Gameplay)`——纯游戏输入；
- **悬浮快捷菜单开 → `UiBrowse` / `UiTyping`**：`UShellFloatingQuickButton`
  按"是否有活动打字面（输入接管 + 面片可见）"条件化——有 → `UiTyping`
  （fill/继续打字要焦点），无 → `UiBrowse`（只浏览 UI、不抢键盘，WASD 保留）；
  菜单关且本次切到过 `UiBrowse` → 回落 `Gameplay`。
...
- `SetShellUIFocus(bool)`：**签名保留**（兼容既有蓝图调用），内部退化为薄映射——
  有活动打字面 → `UiTyping`，否则 `UiBrowse` / `Gameplay`；
- 呈现态切换**不再走 `SetShellUIFocus`**：`ApplyShellPresentation` 直接
  `RequestState(UiTyping / Gameplay)`；
- 全屏 HUD 范式的 `ApplyUIInputMode` / `ApplyGameInputMode` 已标记 deprecated，
  转发到管理器的 executor（`ApplyUiOnly` / `ApplyGameOnly`）。

请求语义：同帧多次请求合并为一次（下一帧应用），重复请求当前态直接忽略，
应用幂等；状态切换经 `OnStateChanged(NewState, PrevState)` 广播，宿主可订阅做
UI 联动。视口回焦断言（坑七）同步收口：仅在 `UiTyping` 且有
活动打字面时由管理器重断言焦点。
...
| 世界屏幕面片无法点击/滚动 | 已修复：`Screen` 需 `QueryOnly + Block Visibility`（`NoCollision` 会对交互射线隐形）；点击/滚轮由 `SetInputActive(true)` 自包含转发；输入模式须 `SetShellUIFocus(true)`（`SetHideCursorDuringCapture(false)`） |
...
3. 输入状态经 `UShellInputStateManager::RequestState` 声明式接管（`Gameplay` / `UiBrowse` / `UiTyping` 三态，见 §6.7）——不再自己拼 `FInputMode*`；`SetShellUIFocus` 保留为兼容薄映射（有打字面→UiTyping，否则 UiBrowse/Gameplay）；
...
3. 输入模式收敛到对方 PC：漏抄 `SetShellUIFocus`（尤其 `SetHideCursorDuringCapture(false)`）点击体验像坏掉（坑二）；
