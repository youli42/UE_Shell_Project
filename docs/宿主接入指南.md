...
完整接线直接抄壳工程 `Source/UE_Shell_Project/Gameplay/ShellProjectPlayerController.cpp`：
...
> `Source/UE_Shell_Project/Gameplay/ShellProjectCommands.h/.cpp` 与其在
...
壳工程 `Source/UE_Shell_Project/Menu/ShellMenuGameMode.cpp` 只注册宿主命令（`BeginPlay`）：
...
> ✅ **原「已知问题」已全部修复（2026-09 架构整理）**：
> 1. **可视化面片（`ProxyMesh`）轴对齐** —— 旋转修正为 `FRotator(0,90,-90)`
>    （等价 `MakeRotationFromAxes(Forward=+Y, Right=+Z, Up=+X)`，宽→Y/高→Z/法线→X
>    与 Widget 面片同向），缩放改按平面局部轴 `(W/100, H/100, 1)`；旋转在
>    `ApplyConfiguration` 每次注册时幂等应用，旧地图里保存的错误旋转会被代码自动纠正。
> 2. **显示器可点击/可滚动** —— 三处根因一并修复：
>    - `Screen`（WidgetComponent）原先 `NoCollision`，对交互组件的 Visibility 物理
>      射线「隐形」——已改为 `QueryOnly + 仅挡 Visibility`（其余通道忽略，不影响游戏碰撞）；
>    - `UShellWorldScreen::SetInputActive(true)` 自包含挂载指针输入：运行时构建
>      IA（左键/滚轮）+ IMC（默认优先级 2，可配 `PointerMappingPriority` 避开宿主冲突）→ Owner Actor `EnableInput` → 绑定转发到
>      `InteractionComponent->PressPointerKey/ScrollWheel`，世界面片像普通 UI 一样
>      可点击、可滚轮；
>    - 输入模式：宿主 PC `SetShellUIFocus(true)`，或用 `bManageInputMode=true`（默认）让 Actor 自接管通用输入模式：
>      `GameAndUI + SetHideCursorDuringCapture(false)`，按住左键时光标不消失、
>      视口不捕获（默认构造的 `bHideCursorDuringCapture=true` 会破坏点击体验）。
>    菜单场景 Tab 也不再弹出全屏 HUD 终端（世界屏是唯一登录入口）。

### 5.3 游戏关卡 GameMode：登录守卫 + HUD

抄壳工程 `Source/UE_Shell_Project/Gameplay/ShellGameplayGameMode.cpp`：

- 下一 tick 校验 `UShellSessionSubsystem::IsLoggedIn()`，未登录 `ClientTravel`
  弹回菜单；
- 已登录创建 `UShellSessionHUD`（登录态标签）加入视口。输入模式/光标/焦点策略
  经 `UShellInputStateManager::RequestState` 声明式接管（见 §6.7），由
  `AShellProjectPlayerController` 的呈现态驱动：OnPossess 应用默认呈现态
  （HeldInHand → `Gameplay`，GameOnly + 隐藏光标），GameMode 不再直接设置。
...
### 6.6 输入键归属矩阵（流控键所有权，2026-09-06 新增）

**问题背景**：插件面板在全屏范式下消费 Tab（命令补全/空输入关闭）与 ESC（关闭终端）。
世界面片架构下终端持有键盘焦点时，这两个键会被面板吞掉、到不了宿主的游戏输入
绑定——宿主的呈现切换/退出随之失效。键的归属是宿主的范式决策，由配置声明：

```ini
; Config/DefaultGame.ini —— 本宿主声明"宿主自持"范式
[/Script/Shell_UE.ShellSettings]
FlowKeyMode=HostOwned
```
...
| 键 | 归属 | 行为 |
| --- | --- | --- |
| 可打印字符/方向/Home/End/回车/Ctrl+C | 终端面板 | 文本编辑与 ^C 中断（不受范式影响） |
| Tab | **宿主**（双管道：面板焦点时经 OnHostFlowKey 委托；无焦点时走 IMC_Shell 游戏输入） | 循环呈现态（HeldInHand→InputWindow）；面板焦点下不触发命令补全 |
| ESC | **宿主**（双管道同上） | InputWindow 态退出回 HeldInHand；其余态忽略 |
| 左键/滚轮 | 世界屏（指针悬停命中本屏时） | 面片 UI 点击/滚动 |
| WASD/空格/鼠标视角 | 角色（InputWindow 态让位） | 移动/跳跃/视角 |
| 快捷指令热键（M5，如 Ctrl+Alt+K） | **双管道**：面板焦点时由 `SShellTerminalPanel::OnPreviewKeyDown` 经 `TryConsumeHotkey` 消费；无焦点时走宿主 PC 构建的热键 IMC（优先级 3） | 触发绑定的快捷指令（ExecuteNow→提交 / FillInput→填入）；交互提示激活期间不消费 |
| 快捷菜单开关键（M5，默认 LeftAlt） | **宿主 PC**（独立 IMC，优先级 3；键位取 `UShellSubsystem::GetMenuToggleChord()`，改键经 `OnQuickCommandsChanged` 重建）→ 直接调悬浮按钮 `ToggleMenu()` | 唤起/关闭世界空间快捷菜单；终端关闭时也生效 |

> M5 热键的让位规则集中在 `Shell::Terminal::ShouldConsumeHotkeyInPanel`（提示激活 / 输入框非空 / 无修饰键 → 让位给打字）。保留键（Tab/ESC/Enter/BackSpace/空格/左键）不可绑定，由 `UShellSettings::ReservedKeys` 声明。

原则（防同类问题复发）：**命令层只广播状态、面板只处理文本编辑键、
流控键归宿主范式；输入模式 / 光标 / 焦点策略已由 `UShellInputStateManager`
声明式接管（`RequestState`，见 §6.7）**。插件任何"替宿主做决定"的行为
（隐式创建表面、越权输入模式、吞流控键）都应视为缺陷上报/修复。

**HostOwned 的实现要点**（踩坑记录）：面板对 Tab/ESC 既不透传给游戏输入
（PIE-in-editor 下未处理键会被编辑器全局命令兜底层
`UnhandledKeyDownEventHandler` 截胡——Tab 恰是编辑器绑定键，独立运行
游戏则正常，行为随环境不一致），也不自持语义，而是**捕获后经
`UShellSubsystem::OnHostFlowKey` 委托把语义交还宿主**。面板持有键盘
焦点（InputWindow 态）走委托管道、无焦点走游戏输入 IMC 管道，两路互补
且单次触发；独立运行与 PIE 内行为完全一致。

### 6.7 输入状态管理器（M6）：输入姿态的声明式裁决

**M6 起，输入模式 / 光标可见性 / 键盘焦点不再由各调用点自己拼 `FInputMode*`
或直接 `FocusTerminal()`**，而是先声明"当前处于什么输入姿态"，由插件侧
`UShellInputStateManager`（`UGameInstanceSubsystem`，跨关卡存活）统一裁决并落地。
宿主只表达意图，不碰 `SetInputMode` / `SetKeyboardFocus`：

```cpp
#include "Shell/Input/ShellInputStateManager.h"
// 取管理器（GameInstance 子系统）
UShellInputStateManager* Mgr = GetGameInstance()->GetSubsystem<UShellInputStateManager>();
Mgr->RequestState(FName("UiTyping"));   // 打字态：输入模式/光标/焦点由管理器按政策落地
```

内置三态（政策也可被 `UShellSettings::InputStateTable` 指向的 DataTable 数据驱动
覆盖；表缺失/加载失败回落内置并告警，`InputStateTable` 默认指向
`/Shell_UE/Shell/Data/DT_ShellInputStates`）：

| 状态 | 输入模式 | 光标 | 键盘焦点 | 壳工程宿主映射 |
| --- | --- | --- | --- | --- |
| `Gameplay` | `GameOnly` | 隐藏 | 无 | **HeldInHand**（手持面板，输入接管关闭）；`RequestGameplayState()` |
| `UiBrowse` | `GameAndUI` | 显示 | 无（不抢键盘，WASD 保留） | 悬浮快捷菜单开但无活动打字面；`RequestUiState()` 解析所得 |
| `UiTyping` | `GameAndUI` | 显示 | 终端文本框（存在活动打字面时） | **InputWindow**（面前大窗口）；悬浮菜单开且有活动打字面；`ShowLoginScreen` |

壳工程的姿态映射（全部落在 `AShellProjectPlayerController` 与悬浮按钮两处）：

- **InputWindow → UI 呈现态**：`ApplyShellPresentation` 在面前态调
  `RequestUiState()`——管理器按活动打字面裁决：有 → `UiTyping`（焦点进终端可打字），
  终端控件未就绪 → `UiBrowse`（不虚标状态）；
- **HeldInHand → `Gameplay`**：同函数在手持态调 `RequestGameplayState()`——纯游戏输入；
- **悬浮快捷菜单开 → `UiBrowse` / `UiTyping`**：`UShellFloatingQuickButton` 也调
  `RequestUiState()`（不再自己手写"活动打字面"三条件，避免漏项）；
  菜单关且本次解析为 `UiBrowse` → `RequestGameplayState()` 回落纯游玩。

**打字面判定只有一处**：管理器的 `HasActiveTypingSurface()`（输入接管 +
面片可见 + 终端控件非空）。宿主与插件调用点都只表达意图，不再各自算条件。

**焦点窃取不变式**（`Shell::InputState::ShouldStealKeyboardFocus`）：只有政策的
`Focus == TerminalText` **且**存在活动打字面才允许 `SetKeyboardFocus`。全插件唯一
的 raw `FSlateApplication::SetKeyboardFocus` 在管理器的 `ApplyFocusForState()` 一处，
且该函数**所有调用点**都先过同一条不变式（`ApplyState`、同态补断言、视口回焦）
——"UI 开着但吃 WASD"、焦点漂走回不来这一类 bug 从结构上被堵死。

旧入口的去向（迁移清单见插件设计文档
`Plugins/Shell_UE/RawContent/doc/13-输入状态管理器.md` §5）：

- `SetShellUIFocus(bool)`：**签名保留**（兼容既有蓝图调用），内部改为只表达意图——
  true → `RequestUiState()`，false → `RequestGameplayState()`；
- 呈现态切换**复用同一入口**：`ApplyShellPresentation` 直接调
  `SetShellUIFocus(PresentationState == InputWindow)`——"两入口判据一致"从此是
  **结构事实**（只有一份实现），不再靠两处代码长得一样来维持
  （原 `IsActiveWorldScreenTyping` 已删除，宿主不再自摸世界屏）；
- 全屏 HUD 范式的 `ApplyUIInputMode` / `ApplyGameInputMode` 已标记 deprecated，
  转发到管理器的 executor（`ApplyUiOnly` / `ApplyGameOnly`）。

请求语义：同帧多次请求合并为一次（下一帧应用），应用幂等；重复请求当前态
**不重复应用输入模式/光标**，仅当存在**未竟焦点意图**（要终端焦点但尚未落地，
例如请求时打字面还没就绪）时才补一次焦点断言——既保证"状态相同 ≠ 效果已达成"，
也避免把焦点从别的 UI 手里抢回来。状态切换经 `OnStateChanged(NewState, PrevState)`
广播，宿主可订阅做 UI 联动。视口回焦断言（坑七）同步收口：不变式成立时
（政策要终端焦点 + 有活动打字面）由管理器重断言焦点。
...
| 世界屏幕 `ProxyMesh` 面片/朝向与屏幕不符 | 旋转已修正为 `FRotator(0,90,-90)` 且在 `ApplyConfiguration` 注册时幂等应用（旧地图保存的错误旋转会被自动纠正）；若仍异常，确认组件代码未被旧序列化值覆盖 |
| 世界屏幕面片无法点击/滚动 | 已修复：`Screen` 需 `QueryOnly + Block Visibility`（`NoCollision` 会对交互射线隐形）；点击/滚轮由 `SetInputActive(true)` 自包含转发；输入状态须经 `RequestUiState()`（或兼容入口 `SetShellUIFocus(true)`）进入 UI 态——`GameAndUI` + 不隐藏捕获光标由管理器政策落地 |
...
| Alt 后 WASD 不能移动 | 修复：悬浮按钮开菜单已按表面活跃度条件化（HeldInHand→UiBrowse 不抢焦点）；若仍出现，检查当前输入状态是否停在 UiTyping 或焦点仍在终端文本框（见 §6.7） |
...
| 文档章节 | 壳工程参考文件 |
|---|---|
| 3 调用 Shell | `Source/UE_Shell_Project/Gameplay/ShellProjectPlayerController.h/.cpp` |
| 4 注册命令（C++） | `Source/UE_Shell_Project/Gameplay/ShellProjectCommands.h/.cpp`（`hello`/`roll` 演示）、`Menu/ShellMenuGameMode.cpp`（接线） |
...
| 5 开始页面 | `Source/UE_Shell_Project/Menu/ShellMenuGameMode.cpp`、`ShellGameplayGameMode.cpp`、`Config/DefaultEngine.ini` |
| 5.2.1 世界终端屏幕 | 插件 `Plugins/Shell_UE/Source/Shell_UE/.../Shell/WorldScreen/`（`UShellWorldScreen`、`AShellWorldScreenActor`、`UShellWidgetInteractionComponent`） |
| 6 GAS | `Source/UE_Shell_Project/Gameplay/ShellProjectPlayerState.cpp`、`ShellProjectCharacter`、`ShellProjectAttributeSet`、`ShellProjectAbility`、`ShellFireballProjectile`、`Config/DefaultGameplayTags.ini` |
...
| 6.6 输入键归属 / 世界屏交互与焦点 | **`docs/输入与焦点踩坑实录.md`**（五坑速查、证据链、架构原则、验证工具箱） |
| 6.7 输入状态管理器（M6） | 插件 `Plugins/Shell_UE/Source/Shell_UE/.../Shell/Input/`（`ShellInputStateTypes` / `ShellInputStateCore` / `ShellInputStateManager`）、`ShellSettings::InputStateTable`；设计文档 `Plugins/Shell_UE/RawContent/doc/13-输入状态管理器.md`；宿主呈现态 `ShellProjectPlayerController::ApplyShellPresentation` |
...
## 11. 迁移到其他工程（复用清单，2026-09-06）

"世界终端屏"是三层组成，迁移对象与方式不同——**插件和组件是"搬"（组件已并入插件），接线是"抄"，配置是"重做"**。

### 11.1 三层组成

| 层 | 内容 | 迁移方式 |
|---|---|---|
| 插件 Shell_UE（终端本体） | 命令系统、`UShellSubsystem`、Slate 面板、CRT 材质、字体、数据表 | **整体搬运**：submodule 挂载（推荐，可跟上游）或复制 `Plugins/Shell_UE` 后在 `.uproject` 启用 |
| 世界屏组件（`Source/.../Screen/`） | `UShellWorldScreen`、`AShellWorldScreenActor`、`UShellWidgetInteractionComponent` | **插件自带**（`Plugins/Shell_UE/Source/Shell_UE/.../Shell/WorldScreen/`）：宿主依赖 `Shell_UE` 模块即可，无需复制源码（Build.cs 无需再补 Screen 依赖）
| 接线代码（PC/Character/GameMode） | IMC 挂载、流控键订阅、`SetShellUIFocus`、焦点断言、姿态动画 | **抄写模式**：对方游戏有自己的 PC/角色，按 §3–§5 五段协议抄，勿整类复制 |

### 11.2 接线五段协议（抄写清单）

1. `BeginPlay` 挂载插件 `IMC_Shell`（优先级 0）+ 绑定 `IA_TerminalToggle`（TSoftObjectPtr 默认值指向 `/Shell_UE/Shell/Input/`，随插件走）；
2. 订阅 `UShellSubsystem::OnHostFlowKey`（Tab/ESC 委托）+ 运行时构建自己的 Escape IMC；
3. 输入状态经 `UShellInputStateManager` 声明式接管（`Gameplay` / `UiBrowse` / `UiTyping` 三态，见 §6.7）——不再自己拼 `FInputMode*`，也不自己判断打字面：调 `RequestUiState()` / `RequestGameplayState()` 表达意图即可；`SetShellUIFocus` 保留为兼容入口；
4. 世界屏点击落屏时 `FocusTerminal()` 断言键盘焦点（虚拟用户/键盘用户双焦点问题，见踩坑实录坑二）；
5. 组件 `SetInputActive(true)` 一键开启指针转发（内置绑定/多屏仲裁/修饰键同步）。

### 11.3 必须重配的清单（漏了就"看起来坏了"）

| 项 | 位置 | 漏配后果 |
|---|---|---|
| `FlowKeyMode=HostOwned` | 对方 DefaultGame.ini | 不配则 Tab/ESC 被面板吞（全屏范式默认） |
| `Accounts`（登录账户） | ShellSettings | 只能靠 fallback `alice` |
| `bAutoTravelOnLogin` + `GameplayLevelPath` | ShellSettings | login 后跳转目标要指**对方**的关卡 |
| 打包 `DirectoriesToAlwaysCook`（字体/数据/输入/地图） | 对方 DefaultGame.ini | **插件 Config 不并入打包配置链**（见 §10）：不抄则打包后终端无文字/数据缺失 |
| 地图与 GameMode | DefaultEngine.ini | `AShellWorldScreenActor` 要摆进**对方**的开始关卡 |
| EnhancedInput 类 | DefaultInput.ini | `DefaultPlayerInputClass/DefaultInputComponentClass` 设为 EnhancedInput |

### 11.4 迁移后四大坑（详见踩坑实录）

1. `Screen` 碰撞必须 `QueryOnly + Block Visibility`（组件已内置；手改碰撞会让交互射线"隐形"，坑一）；
2. Tab/ESC 必须在链上认领（订阅 `OnHostFlowKey` / 挂 IMC）：PIE 内不认领会被编辑器兜底截胡；对方项目已有 Tab/ESC 绑定时先按 §6.6 键归属矩阵合并（坑五）；
3. 输入状态收敛到管理器：漏调 `RequestUiState()`（或兼容入口 `SetShellUIFocus(true)`）点击体验像坏掉——`GameAndUI` + 不隐藏捕获光标由管理器政策落地（坑二）；
4. 构建卫生：改完 C++ 核对 Binaries 为基础名 DLL 而非 `-000X` 热重载变体（坑三）。

### 11.5 最小可用路径

"登录页世界屏 + 可打字可点击"：插件挂载（含世界屏组件）+ 对方开始关卡摆一个 `AShellWorldScreenActor` + BeginPlay 调 `ShowLoginScreen`（内部已完成 SetInputActive/聚焦/输入模式）——半天内可跑通。Gameplay 两态呈现机（Tab 循环：HeldInHand↔InputWindow，已删 HUD 视口态）是可选增强，按需抄。
