...
**打字面判定只有一处**：管理器的 `bHasActiveTypingSurface()`（输入接管 +
...
- 呈现态切换同源：`ApplyShellPresentation` 也调 `RequestUiState()` /
  `RequestGameplayState()`，两个入口判据完全一致（原 `IsActiveWorldScreenTyping`
  已删除，宿主不再自摸世界屏）；
...
**不重复应用输入模式/光标，但会补一次焦点断言**——"状态相同"不等于"效果已达成"
（请求当时打字面可能尚未就绪）。状态切换经 `OnStateChanged(NewState, PrevState)`
